    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <label className="flex items-center gap-2 text-xs text-zinc-500">
          ...
          <select
            ...
            className="bg-zinc-800 text-zinc-200 text-xs rounded-lg px-2 py-1.5 border border-zinc-700"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <TopList
          ...
          icon={<ImageIcon size={15} className="text-zinc-500" />}
          ads={topStatic}
        ...
        <TopList
          title="Top 10 — Wideo"
          icon={<Video size={15} className="text-zinc-500" />}
          ads={topVideo}
          attribution={attribution}
          onSelect={setSelected}
        />
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
        <h3 className="text-sm font-medium text-zinc-200 mb-3">
          Wszystkie reklamy <span className="text-zinc-500 font-normal">· klik = podgląd kreacji</span>
        </h3>
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-zinc-600">{ads.length} reklam</span>
      </div>
      {ads.length === 0 ? (
        <p className="text-xs text-zinc-600 py-6 text-center">Brak reklam w tym zakresie</p>
      ) : (
          ...
          {ads.map((ad, i) => {
            ...
            return (
              <li key={ad.adId}>
                <button
                  onClick={() => onSelect(ad)}
                  className="w-full flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-zinc-800/50 transition-colors text-left"
                >
                  <span className="relative">
                    <CreativeThumb ad={ad} size={44} />
                    <span className="absolute -top-1 -left-1 w-4 h-4 rounded bg-zinc-700 text-[10px] text-zinc-200 flex items-center justify-center font-semibold">
                      {i + 1}
                    </span>
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block text-sm text-zinc-200 truncate">{ad.adName}</span>
                    <span className="block text-[11px] text-zinc-500 truncate">
                      {ad.creative?.format || '—'} · {ad.shop}
                    </span>
                  </span>
                  <span className="text-right shrink-0">
                    <span className="block text-[10px] uppercase text-zinc-600">Wydatki</span>
                    <span className="block text-xs text-zinc-300">{formatNumber(ad.spend, 2)} zł</span>
                  </span>
                  <span className="text-right shrink-0 w-20">
                    <span className="block text-[10px] uppercase text-zinc-600">ROAS</span>
                    <span className={cn('block text-xs font-semibold', att.roas >= 1 ? 'text-emerald-400' : 'text-red-400')}>
                      {att.roas.toFixed(2)}×
                    </span>
                  </span>
                  <span className="text-right shrink-0 w-14">
                    <span className="block text-[10px] uppercase text-zinc-600">Zakupy</span>
                    <span className="block text-xs text-zinc-300">{att.purchases}</span>
                  </span>
                </button>
              </li>
            );
