...
const TIER_BADGE: Record<string, string> = {
  core: 'bg-primary-soft text-primary-ink border-primary/20',
  standard: 'bg-surface-2 text-ink-soft border-line',
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-ink">KPI i widgety</h1>
          <p className="text-sm text-ink-muted">Twórz i zarządzaj KPI, dodawaj KPI oraz gotowe widgety na dashboard.</p>
        </div>
        <button
          onClick={openNew}
      ...
      {loading ? (
        <div className="text-ink-faint animate-pulse">Ładowanie…</div>
      ) : (
        <>
          {/* Twoje KPI */}
          <div className="space-y-4">
            <h2 className="section-title">Twoje KPI</h2>
            {kpis.length === 0 ? (
              <div className="flex items-center justify-between gap-3 rounded-2xl border border-dashed border-line px-4 py-5">
                <p className="text-sm text-ink-muted">Nie masz jeszcze własnych KPI.</p>
                <button onClick={openNew} className="btn-primary px-3 py-1.5 text-xs shrink-0">
              ...
              categories.map(cat => (
                <section key={cat} className="space-y-2">
                  <h3 className="stat-label">{cat}</h3>
                      ...
                      <div key={k.id} className="card p-4 flex flex-col gap-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            {k.description && <div className="text-xs text-ink-muted mt-0.5 line-clamp-2">{k.description}</div>}
                          </div>
                          <span className={`chip shrink-0 ${TIER_BADGE[k.tier] || TIER_BADGE.standard}`}>
                            {TIER_LABEL[k.tier] || k.tier}
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[10px] px-2 py-0.5 rounded bg-surface-2 text-ink-soft">{VALUE_TYPE_LABEL[k.value_type] || k.value_type}</span>
                          <span className="text-[10px] px-2 py-0.5 rounded bg-surface-2 text-ink-soft">{k.query_spec?.chart_type}</span>
                          <span className="text-[10px] px-2 py-0.5 rounded bg-surface-2 text-ink-soft">{k.query_spec?.y_axis}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-auto">
                          <button
                            onClick={() => addKpiToDashboard(k)}
                            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-colors ${
                              addedId === k.id ? 'bg-emerald-50 text-emerald-700' : 'bg-surface-2 hover:bg-line/60 text-ink'
                            }`}
                          >
                          ...
                          {canEdit(k) && (
                            <>
                              <button onClick={() => openEdit(k)} className="p-1.5 rounded-lg bg-surface-2 hover:bg-line/60 text-ink-soft transition-colors" title="Edytuj">
                                <Pencil size={13} />
                              </button>
                              <button onClick={() => deleteKpi(k.id)} className="p-1.5 rounded-lg bg-surface-2 hover:bg-red-50 text-ink-soft hover:text-red-600 transition-colors" title="Usuń">
                                <Trash2 size={13} />
                              </button>
                            </>
                          )}
          ...
          <div className="space-y-4">
            <div>
              <h2 className="section-title">Widgety wbudowane</h2>
              <p className="text-xs text-ink-muted">Gotowe widgety — kliknij, aby dodać na dashboard. Nie można ich edytować.</p>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {builtinByCat[cat].map(w => (
                    ...
                    >
                      ...
                      <span className={`shrink-0 inline-flex items-center gap-1 text-xs ${addedId === w.type ? 'text-emerald-700' : 'text-ink-muted'}`}>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl card shadow-pop animate-scale-in">
            <div className="flex items-center justify-between px-5 py-3 border-b border-line">
              <h2 className="section-title">{editingId ? 'Edytuj KPI' : 'Nowy KPI'}</h2>
              <button onClick={() => setModalOpen(false)} className="p-1 rounded-lg text-ink-faint hover:bg-surface-2 hover:text-ink transition-colors">
                <X size={18} />
              </button>
            </div>
              ...
              {(['basic', 'query'] as const).map(t => (
                <button
                  ...
                  className={`px-3 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
                    tab === t ? 'border-primary text-ink' : 'border-transparent text-ink-muted hover:text-ink-soft'
                  }`}
                >
            ...
            <div className="p-5 max-h-[62vh] overflow-y-auto">
              {tab === 'basic' ? (
                <div className="space-y-3">
                  <div>
                    <label className="stat-label">Nazwa</label>
                    <input
                      value={draft.name}
                  ...
                  <div>
                    <label className="stat-label">Opis</label>
                    <textarea
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      <label className="stat-label">Kategoria</label>
                      <input
                    ...
                    <div>
                      <label className="stat-label">Poziom (Tier)</label>
                      <select
                        value={draft.tier}
                    ...
                    <div>
                      <label className="stat-label">Typ wartości</label>
                      <select
                ...
                <div className="space-y-4">
                  <div>
                    <label className="stat-label block mb-1.5">Typ wykresu</label>
                    <ChartTypePicker
                  ...
                  <div>
                    <label className="stat-label block mb-1.5">Dane i filtry</label>
            ...
            <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-line">
              ...
              <div className="flex gap-2">
                <button onClick={() => setModalOpen(false)} className="btn-ghost px-3 py-1.5">
                  Anuluj
                </button>
                <button
                  onClick={saveKpi}
